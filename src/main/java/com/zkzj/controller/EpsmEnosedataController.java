...
public class EpsmEnosedataController extends BaseController {
    ...
    public Result epsmEnosedataList() {
        ...
        return ResponseUtil.success(epsmEnosedataList,(long)epsmEnosedataList.size());
    }
     ...
     * @return
     */
    @ApiOperation(value="获取电流数据(分页)", notes="根据url的start来获取从第start条开始，每次取end-start条")
    @ApiImplicitParams({
    ...
    @GetMapping(value = "/epsmEnosedatas/from/{start}/to/{end}")
    public Result selectEpsmEnosedataPage(@ApiParam(name = "start", required = true, value = "第start条记录")@PathVariable("start") Integer start,@ApiParam(name = "end", required = true, value = "第end条记录")@PathVariable("end") Integer end) {
        logger.info("selectEpsmEnosedataPage");
        Assist assist = new Assist();
        assist.setStartRow(start);
        assist.setRowSize(end-start);
        List<EpsmEnosedata> epsmEnosedataList = epsmEnosedataService.selectEpsmEnosedata(assist);
        return ResponseUtil.success(epsmEnosedataList,(long)epsmEnosedataList.size());
    }
